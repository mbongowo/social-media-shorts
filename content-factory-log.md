...
## Batch 27 (2026-09-16)
- SpatialCVLeak — Geospatial Data Science — a rooftop-detection study found that random train/test splits let spatially autocorrelated (near-identical, neighboring) satellite pixels leak between training and test sets, inflating reported accuracy by up to 17 points and misreporting over 99% where it was not earned; spatial block splits, which hold out a contiguous area entirely, avoid the leak. Not a current-events claim, so Gate 1 does not apply. Abriha, D., Srivastava, P.K. and Szabo, S., "Smaller is better? Unduly nice accuracy assessments in roof detection using remote sensing data with machine learning and k-fold cross-validation," Heliyon 9(3):e14045, 2023, DOI 10.1016/j.heliyon.2023.e14045. Verification note: this run's network egress proxy again blocked a direct api.crossref.org query (CONNECT tunnel, 403), so the DOI, title, full author list, journal and the "up to 17 points" / "over 99%" figures were cross-checked via WebSearch triangulation across the publisher's own Heliyon/Cell fulltext page, the NCBI/PMC listing and a second independent WebSearch query, all agreeing, same fallback used in every batch since 16. Scope check: framed on screen and in the manifest as a rooftop-detection case study finding (WorldView-2 imagery, random-forest classifier), not generalized to "all AI models," matching the claim to the study's scope per Rule Zero.
- Layout note: the first render placed the "Random split" label overlapping the hook text because the dot grid was centered too high (UP*2.2). Fixed by moving the grid to UP*1.2 and re-rendered; also replaced a manually positioned highlight Rectangle around the held-out columns (which drew incorrectly) with SurroundingRectangle around the actual held-out Dot mobjects, confirmed clean in the second render.
- Theme rotation note: batch 26 (2026-09-14) used Remote Sensing explained, so this run picked Geospatial Data Science, the theme least recently used (last seen batch 23, 2026-09-07).
- Frame-zero check: first frame measured mean luma 22.9, std 19.2 (well above zero), with the complete two-line hook fully drawn at t=0; confirmed clean via `self.add(hook)` + `self.wait(1)` opening beat.
- Cover note: brightest sampled frame (YAVG 41.47 at t=9.5s) lands with the complete hook, the full 7x7 leakage grid and the complete fact line all fully drawn and legible simultaneously; after the brightness/contrast/saturation lift the exported cover measures mean luma about 31.7, still under the roughly 57 threshold noted in the brief, so this short will read dimmer than the two best-performing covers in feeds. Flagged plainly in the email.

## Note on DoualaSinking.mp4 and PuertoRicoDrought.mp4 (still open, checked again 2026-09-16)
Both still sit at the repo root outside this factory's pipeline (no log entry beyond these notes, no manifest entry, no cover). Left untouched again this run.
